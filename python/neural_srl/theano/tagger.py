...
class BiLSTMTaggerModel(object):
  ...
  def __init__(self, data, config, fast_predict=False):
    ...
    for l in range(self.num_lstm_layers):
      ...
      self.rnn_layers[l] = LSTMLayer(input_dim, 
    ...
    self.crf_layer = CRFLayer(self.lstm_hidden_size, self.label_space_size)
    self.params.extend(self.crf_layer.params)
    ...
    self.scores, self.pred = self.crf_layer.connect(self.inputs[-1])
